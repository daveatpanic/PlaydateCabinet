...
#if ENABLE_Z_BUFFER
		if ( shape->useZBuffer )
			fillQuad_zbuf(bitmap, rowstride, face->p1, face->p2, face->p3, face->p4, (uint8_t)vi);
		else
#endif
			fillQuad(bitmap, rowstride, face->p1, face->p2, face->p3, face->p4, (uint8_t)vi);
	}
	else
	{
#if ENABLE_Z_BUFFER
		if ( shape->useZBuffer )
			fillTriangle_zbuf(bitmap, rowstride, face->p1, face->p2, face->p3, (uint8_t)vi);
		else
#endif
			fillTriangle(bitmap, rowstride, face->p1, face->p2, face->p3, (uint8_t)vi);
	}
}
static inline void
...
drawWireframe(Scene3D* scene, ShapeInstance* shape, uint8_t* bitmap, int rowstride)
{
	...
	for ( f = 0; f < shape->nFaces; ++f )
	{
		...
		if ( shape->useZBuffer )
		{
			drawLine_zbuf(bitmap, rowstride, face->p1, face->p2, 1, 15);
			drawLine_zbuf(bitmap, rowstride, face->p2, face->p3, 1, 15);

			if ( face->p4 != NULL )
			{
				drawLine_zbuf(bitmap, rowstride, face->p3, face->p4, 1, 15);
				drawLine_zbuf(bitmap, rowstride, face->p4, face->p1, 1, 15);
			}
			else
				drawLine_zbuf(bitmap, rowstride, face->p3, face->p1, 1, 15);
		}
		else
		{
#endif
			drawLine(bitmap, rowstride, face->p1, face->p2, 1, 15);
			drawLine(bitmap, rowstride, face->p2, face->p3, 1, 15);

			if ( face->p4 != NULL )
			{
				drawLine(bitmap, rowstride, face->p3, face->p4, 1, 15);
				drawLine(bitmap, rowstride, face->p4, face->p1, 1, 15);
			}
			else
				drawLine(bitmap, rowstride, face->p3, face->p1, 1, 15);
#if ENABLE_Z_BUFFER
		}
#endif
	}
