...
static int scene_draw(lua_State* L)
{
	...
	memset(frame,0,120*200);
	Scene3D_draw(scene, frame, 200);
	
	for ( int y = 0; y < 120; ++y )
	{
		uint8_t rowbuf[400];
		uint8_t* framep = &frame[y*200];
		...
		for ( int x = 0; x < 50; ++x )
		{
			...
			*bitmap = (c1 & 0xc) << 4 | (c2 & 0xc) << 2 | (c3 & 0xc) | (c4 & 0xc) >> 2;
			*(bitmap+52) = (c1 & 0x3) << 6 | (c2 & 0x3) << 4 | (c3 & 0x3) << 2 | (c4 & 0x3);
			++bitmap;
		}
	...
	pd->graphics->markUpdatedRows(0, 239); // XXX
...
static int setPalette(lua_State* L)
{
	...
	float hue = 6 * pd->lua->getArgFloat(1);
	float d = hue - floorf(hue);
	int n = (unsigned int)floorf(hue) % 6;
	
	float r, g, b;

	switch ( n )
	{
		case 0: r=1; g=d;   b=0; break;
		case 1: r=1-d; g=1;   b=0; break;
		case 2: r=0;   g=1; b=d; break;
		case 3: r=0;   g=1-d; b=1; break;
		case 4: r=d;   g=0;   b=1; break;
		case 5: r=1;   g=0;   b=1-d; break;
	}
	
	for ( int i = 0; i < 16; ++i )
	{
		float l = 255.0f * i / 15;
		float w = i > 4 ? (i-4)/11.0f : 0;
		
		palette[3*i+0] = l * (w + (1-w)*r);
		palette[3*i+1] = l * (w + (1-w)*g);
		palette[3*i+2] = l * (w + (1-w)*b);
	}
	
	pd->system->sendMirrorData(2, palette, sizeof(palette));
	return 0;
}

static int resetFrame(lua_State* L)
{
	pd->system->sendMirrorData(0, NULL, 0);
	return 0;
}
...
void register3D(PlaydateAPI* playdate)
{
	...
	if (!pd->lua->addFunction(setPalette, "setPalette", &err) || !pd->lua->addFunction(resetFrame, "resetFrame", &err))
